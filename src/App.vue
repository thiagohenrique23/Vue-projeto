...
<template>
	<main>
		...
		<form @submit.prevent="searchAnime">
			<input type="text" placeholder="Procurar..." v-model="query" @input="handleInput" />
		...
		<div class="results" v-if="search_results.length > 0">
			<div v-for="anime in search_results" class="result">
				...
				<div class="details">
					...
					<button @click="addAnime(anime)" class="button">Adicionar anime</button>
				</div>
			</div>
		</div>
...
.button {
	...
	display: block;
	padding: 0.5rem 1rem;
	background-image: linear-gradient(to right, #c6552d 50%, #0f120b 50%);
	background-size: 200%;
	color: white;
	font-size: 1.125rem;
	font-weight: bold;
